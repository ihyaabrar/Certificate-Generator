...
export default function AcademicBlueTemplate({ data, scale = 1 }: Props) {
  return (
    <div
    ...
    >
      {/* Top blue band */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 80, background: '#1e40af' }} />
      {/* Bottom blue band */}
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 50, background: '#1e40af' }} />
      {/* White content area */}
      ...
      <div style={{
        ...
        <div style={{ fontSize: 36, fontWeight: 'bold', color: '#1e3a8a', letterSpacing: 5, textTransform: 'uppercase' }}>SERTIFIKAT</div>
        <div style={{ width: 80, height: 2, background: '#1e3a8a' }} />
        <div style={{ fontSize: 13, color: '#64748b' }}>Diberikan kepada</div>
        <div style={{ fontSize: 44, fontWeight: 'bold', color: '#1e3a8a', borderBottom: '2px solid #1e3a8a', paddingBottom: 6, minWidth: 360, textAlign: 'center' }}>
          {data.recipientName}
        </div>
        <div style={{ fontSize: 13, color: '#64748b' }}>atas keikutsertaannya dalam</div>
        <div style={{ fontSize: 20, fontWeight: 'bold', color: '#1e40af' }}>{data.eventTitle}</div>
      ...
      <div style={{
        ...
        <CertNo data={data} color="#bfdbfe" />
        <SignatureRow data={data} accentColor="#bfdbfe" textColor="#ffffff" subColor="#93c5fd" />
      </div>
    </div>
  );
}
